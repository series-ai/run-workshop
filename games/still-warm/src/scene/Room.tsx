...
export const Room: FC<RoomProps> = ({
  ...
  return (
    <group name="cellar-room">
      ...
      <group name="lantern-workbench" position={[-1.08, 0.03, 0.63]}>
        <mesh castShadow receiveShadow>
          <boxGeometry args={[0.55, 0.06, 0.8]} />
          <meshStandardMaterial color="#3b2b1d" roughness={0.92} />
        </mesh>
        {[-0.21, 0.21].flatMap((x) =>
          [-0.31, 0.31].map((z) => (
            <mesh
              ...
              receiveShadow
            >
              <boxGeometry args={[0.065, 0.9, 0.065]} />
              <meshStandardMaterial color="#261d15" roughness={0.96} />
            </mesh>
          )),
        )}
      </group>
