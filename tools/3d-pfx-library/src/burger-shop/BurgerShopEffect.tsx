...
import {
  ...
  type BurgerShopBlend,
  type BurgerShopRecipe,
  type BurgerShopTextureId,
} from './types'

const textureCache = new Map<string, Texture>()
const loader = new TextureLoader()

function loadTexture(id: BurgerShopTextureId): Texture {
  const url = BURGER_SHOP_TEXTURE_URLS[id]
...
export function BurgerShopEffect({ recipe }: { recipe: BurgerShopRecipe }) {
  ...
  const layerMeta = useMemo(
    () =>
      recipe.emitters.map((emitter) => {
        const texture = loadTexture(emitter.texture)
        return {
          emitter,
          texture,
          ...
          capacity: Math.max(
            32,
            (emitter.burst?.max ?? 0) + Math.ceil(emitter.rate * Math.max(1, emitter.life.max) * 3) + 24,
          ),
        }
      }),
    [recipe],
