...
const RUNTIME_URL = new URL('assets/runtime/unity-fireball-64.png', MODULE_ROOT)
const PROVENANCE_URL = new URL('assets/unity-fireball-64.provenance.json', MODULE_ROOT)
const OUTPUT_URL = new URL('src/fireballFlipbook.ts', MODULE_ROOT)
const COLUMNS = 8
const ROWS = 8
...
function fail(message) {
  throw new Error(`[build-unity-fireball-flipbook] ${message}`)
}
...
function validateAtlas(image) {
  ...
  if (coverage.some((active) => active < 300)) fail(`one or more flipbook frames are empty: ${coverage.join(', ')}`)
  return coverage
}
...
function main() {
  ...
  if (runtimeSha256 !== provenance.files?.runtime?.sha256) fail('runtime checksum disagrees with unity-fireball-64.provenance.json')
  if (runtime.length !== provenance.files?.runtime?.bytes) fail('runtime byte count disagrees with unity-fireball-64.provenance.json')
  ...
  const output = `// Unity Labs Paris FireBall01 flipbook (CC0-1.0). Do not hand-edit.
// Regenerate with tools/3d-pfx-library/viewer/scripts/build-unity-fireball-flipbook.mjs.

export const PFX_FIREBALL_FLIPBOOK_ATLAS = {
  id: 'unity-fireball-64',
  dataUri: '${dataUri}',
  ...
  provenancePath: 'tools/3d-pfx-library/assets/unity-fireball-64.provenance.json',
} as const
`
