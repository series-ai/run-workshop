...
**RUN** is Series Entertainment's AI game & media creation and publishing platform [run.world](https://run.world). 
You build against the RUN game SDK, develop locally
in a browser sandbox, and publish to the platform with the `rundot` CLI. Each
project under `games/` is self-contained — clone once, then work inside whichever
one you want.

## Getting Started

New here? Read **[GETTING_STARTED.md](GETTING_STARTED.md)** for prerequisites,
one-time repo setup (Git LFS + hooks), CLI authentication, and how to run your
first game starter. Each sub-project then documents its own specifics in its
`README.md`.
...
- [Wreck Yard](games/wreck-yard/) — a multiplayer voxel destruction yard on RUN syncplay: grab and throw voxel bodies, torch them into falling fragments, and float them in a wave tank, all in one deterministic session with rollback and late join. See [games/wreck-yard/README.md](games/wreck-yard/README.md).
...
- [voxel-kit](tools/voxel-kit/) — engine-agnostic voxel destruction core (pure TypeScript, zero runtime deps): stamp, carve, scorch, connected components, tank buoyancy, and world occlusion, with engine-independent math for lockstep simulations. See [tools/voxel-kit/README.md](tools/voxel-kit/README.md).
